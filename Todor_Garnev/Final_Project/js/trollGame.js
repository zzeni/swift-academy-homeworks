...
(function () {
    ...
    const display_scenario = (chosen_option) => {
        const option_name = chosen_option.name
        const option_path = chosen_option.path
        const scenario = story[option_path];

        $('#prompt').empty()
        $('#options').empty()

        option_name ? $('<p>').html('You have chosen <strong>' + option_name + '</strong>').appendTo('#prompt') : null


        $('<p>').html(scenario.prompt).appendTo('#prompt');

        const add_option_button = index => {
            ...
            $('<button>').html(option.name).click((event) => {
    ...
    $("#wrapper h1").append("Forest adventure")
